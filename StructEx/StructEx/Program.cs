...
namespace StructEx
{
    ...
    // int, double, float, bool 타입들은 기본(Primitive Type)이라고 하는데, struct로 정의된 Value Type이다.
    // Value Type은 상속될 수 없으며, 주로 간단한 데이터 값을 저장하는데 사용된다.

    // Reference Type은 Class를 정의해서 만들고, 상속이 가능하고, 좀 더 복잡한 데이터와 기능들을 정의하는
    // 곳에 많이 사용된다.

    // 구조체 : struct라는 키워드를 이용해서 정의한다. 클래스와 같이 메소드, 속성(프로퍼티) 등 거의 비슷한
    //          구조를 가지고 있지만, 상속을 할수 없다.
    //          클래스와 마찬가지로 인터페이스(interface)구현을 할 수 있다.

    // 구조체는 클래스와 달리 복사를 할 때 DeepCopy가 이루어딘다.
