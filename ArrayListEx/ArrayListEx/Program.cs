...
namespace ArrayListEx
{
    /*
        ...
        하지만, 배열을 선언할 때는 배열 크기를 지정하는 반면에 ArrayList는 크기를 지정하지 않는다. 
        요소의 추가, 삭제에 따라서 자동으로 크기를 늘였다 줄였다 한다.

        또한, ArrayList 컬렉션은 모든 타입의 변수를 담을 수 있다.
        (C#에서 제공하는 모든 컬렉션은 모든 타입의 변수를 담을 수 있다. 그 이유는 컬렉션의 요소들은
        object 타입으로 저장되기 때문)
    */
    ...
    {        
        ...
        static void Main(string[] args)
        {
            ...
            // RemoveAt(index) 리스트의 해당 인덱스를 찾아 제러
            // Remove(data)매개변수로 전달된 data를 찾아서 제거
            arrayList.RemoveAt(1);  //arrayList.Remove(2)

            // insert (index, data) index위치에 data를 삽입
            arrayList.Insert(1,2.2f);   // 1, 2.2, 100
            arrayList.Add("ABC");       // 1, 2.2, 100, ABC
            arrayList.Add("가나다");    // 1, 2.2, 100, ABC, 가나다

            // 컬렉션의 모든 요소들은 object 타입이기 때문에 아래에서 object  obj가 가능하다.
