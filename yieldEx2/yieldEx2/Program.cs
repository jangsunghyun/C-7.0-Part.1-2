...
IEnumerator는 Current(속성), MoveNext() (메소드), Rest() (메소드) 3개의 멤버로 이루어져 있다.
따라서,  Enumerator가 되기 위해서는 Current와 MoveNext() 를 반드시 구현 해야 함.
...
컬렉션 타이이나 또는 Enumerable 클래스에서 GetEnumerator() 메소드를 구현하는 방법으로
yield 키워드를 사용할 수 있다.

GetEnumerator() 메소드에서 yield return 사용하면 컬렉션 데이터를 순차적으로 하나씩 넘겨주는 
코드를 구현할 수 있고, 리턴타입은 IEnumerator인터페이스를 리턴할 수 있다.
