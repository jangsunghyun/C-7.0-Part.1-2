...
{
    //기반 클래스(부모) 의 인자 생성자에게 파생(자식) 클래스가 인자를 전달하는 방법
    // base 키워드기반 클래스를 가리키는 키워드이다. 따라서 기반클래스의 멤버를 접근할때 사용 할 수 있다.
    // sealed 키워드(한정자)를 이용하면 상속을 봉인하게되어 상속이 불가능 하도록 클래스를 선언하는 방법이다.
